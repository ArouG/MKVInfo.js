...
var mkv = function(opts, fcb) {
    ...
    ********************************************************************************************************/
        ...
        info.offset = 0;
        info.tracks=[];
        info.JUNKS=[];                                    //  for stocking JUNKS boxes to be deleted after replacing values !
    ...
    var MKVebml = {};
    MKVebml.parse = function(handle, callback) {
            ...
            ***********************************************************************************************/
            ...
            function completeval(ind, Boxes, cplcb){
                var offset=info.noVal[ind].offset;
                var type = info.noVal[ind].type;
                var nbB = info.noVal[ind].nbB;
                readBytes(nbB, offset, function rbcpl(err, buffer){
                    if (err){
                        cplcb(err);
                    } else {
                        if (nbB < info.lengthmax){
                            if (type == "s" || type == "8"){
                                info.noVal[ind].value = litCar(buffer, 0, nbB);
                            } 
                            if (type == "b"){
                                info.noVal[ind].value = litHex(buffer, 0, nbB).toUpperCase();
                            }
                        } else {
                            info.noVal[ind].value = "Too long, sorry !";    
                        }
                        ind++;
                        if (ind < info.noVal.length){
                            completeval(ind, Boxes, cplcb);
                        } else {
                            cplcb(null, Boxes);
                        }
                    }
            ...
            function readAtoms(offset, ascendance, Boxes, racb){
                ...
                var nbBtoRead = info.filesize - offset;
                var nbB = Math.min(20,nbBtoRead);                               // 4 for tagStr + 16 for dataSize max 
                readBytes(nbB, offset, function rbreadAtoms(err,buffer){
                    if (err){
                        racb(err);
                    } else {
                        var start = 0;
                        ...
                        if (length > nbB) {   // theorical impossible
                            racb("Unrepresentable length: " + length + " 0x" + litHex(buffer,start,length) + " at offset : " + offset);
                        }
                        ...
                        if (length+tagObj.next > nbB) {
                            racb("Unrepresentable length: " + length + " 0x" + litHex(buffer,start,length) + " at offset : " + offset+start);
                        } else {
                            tagObj.next += length; 
                        }
                        ...
                        if (info.offset >= info.filesize) {
                            ...
                            readAtoms(info.offset, ascendance, Boxes, function readAtoms_in_readAtoms(err,Boxes){
                                if (err){
                                    racb(err);    
                                } else {
                                    racb(null, Boxes);
            ...
            info.offset = 0;
            readAtoms(info.offset, [0], function(err, Boxes) {
                if (err){
                    callback(err);
                } else {
                    atoms=[];
                    if (info.noVal.length > 0) completeval(0, Boxes, function(err, Boxes){
                        if (err){
                            callback(err);
                        } else {
                            ...
                            info.JUNKS=[];
                            info.noVal=[];
                            //console.log(JSON.stringify(Boxes));    // debugging
                            for (var v = 0; v < Boxes[0].children.length; v++) atoms.push(Boxes[0].children[v]);                                
                            Boxes=null;
                            // analysis of atoms and complete info :
                            //
                            info.isEBML = -1;
                            info.isSegment = -1;
                            info.isInfo = -1;
                            ...
                            for (var k=0; k<atoms.length; k++){
                                if (atoms[k].name == "EBML") info.isEBML=k;
                                if (atoms[k].name == "Segment") info.isSegment=k;    
                            }
    ...
    handle.open(options.file, function(err) {
        if (err) {
            return fcb('Could not open specified file');
        }
        MKVebml.parse(handle, function(err, tags) {
            fcb(err, tags);
        });
    });
}; // MP4Tag.parse ! ??
